      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(
                    'My Profile',
                    style: GoogleFonts.poppins(color: AppColors.darkBrown, fontWeight: FontWeight.bold, fontSize: 28),
                  ),
                ),
                GestureDetector(
            ...
            _menuButton(icon: Icons.menu_book_outlined, label: 'Booking History', onTap: () => widget.onNavigateToTab(1)),
            const SizedBox(height: 14),
            _menuButton(icon: Icons.smart_toy_outlined, label: 'PaladCare', onTap: () => widget.onNavigateToTab(3)),
            const SizedBox(height: 14),
            _menuButton(
...
class _AuthDialogCard extends StatelessWidget {
  final String title;
  final Widget child;

  const _AuthDialogCard({required this.title, required this.child});
...
class _OtpDialog extends StatefulWidget {
  ...
  const _OtpDialog({required this.email, required this.purpose, required this.onResend});
...
class _CreatePasswordDialog extends StatefulWidget {
  ...
  const _CreatePasswordDialog({required this.verificationToken});
...
class _ProfileEditDialogState extends State<_ProfileEditDialog> {
  ...
  Future<void> _startChangePassword() async {
    ...
    final success = await showDialog<bool>(
      context: context,
      barrierDismissible: true,
      builder: (_) => _CreatePasswordDialog(verificationToken: token),
    );
    if (success == true) {
            ...
            Row(
              children: [
                Expanded(
                  child: Text(
                    'My Profile',
                    style: TextStyle(color: AppColors.darkBrown, fontWeight: FontWeight.w800, fontSize: 26),
                  ),
                ),
                GestureDetector(
